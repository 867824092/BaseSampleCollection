        ...
        private Dictionary<string, string> GetResources(string culture) {
            return _resourcesCache.GetOrAdd(culture, _ => {
                ...
                if (File.Exists(_searchedLocation)) {
                    var builder = new ConfigurationBuilder().AddJsonFile(_searchedLocation, optional: true, reloadOnChange: true);
                    value = new Dictionary<string, string>(builder.Build().AsEnumerable());
                    if(value.Count == 0)
                        _logger.LogWarning($"json content is null, path: {_searchedLocation}");
